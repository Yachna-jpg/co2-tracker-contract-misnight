...
import {
  CarbonCreditAPI,
  type CarbonCreditCircuitKeys,
  type CarbonCreditProviders,
  type DeployedCarbonCreditAPI,
} from '../../../api/src/index';
...
import { CarbonCreditPrivateState } from '@midnight-ntwrk/carbon-credit-contract';
import { inMemoryPrivateStateProvider } from '../in-memory-private-state-provider';
import { NetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import type { UnboundTransaction } from '@midnight-ntwrk/midnight-js-types';

/**
 * An in-progress carbon credit tracker deployment.
 */
...
/**
 * A deployed carbon credit tracker deployment.
 */
export interface DeployedBoardDeployment {
  readonly status: 'deployed';

  /**
   * The {@link DeployedCarbonCreditAPI} instance when connected to an on network carbon credit tracker contract.
   */
  readonly api: DeployedCarbonCreditAPI;
}

/**
 * A failed carbon credit tracker deployment.
 */
...
export interface DeployedBoardAPIProvider {
  ...
  /**
   * Joins or deploys a carbon credit tracker contract.
   *
   * @param contractAddress An optional contract address to use when resolving.
   * @returns An observable board deployment.
   *
   * @remarks
   * For a given `contractAddress`, the method will attempt to find and join the identified carbon credit tracker
...
/**
 * A {@link DeployedBoardAPIProvider} that manages carbon credit tracker deployments in a browser setting.
 *
 * @remarks
...
export class BrowserDeployedBoardManager implements DeployedBoardAPIProvider {
  ...
  #initializedProviders: Promise<CarbonCreditProviders> | undefined;
  ...
  private getProviders(): Promise<CarbonCreditProviders> {
  ...
  private async deployDeployment(deployment: BehaviorSubject<BoardDeployment>): Promise<void> {
    try {
      ...
      const api = await CarbonCreditAPI.deploy(providers, this.logger);
  ...
  private async joinDeployment(
    ...
    try {
      const providers = await this.getProviders();
      const api = await CarbonCreditAPI.join(providers, contractAddress, this.logger);
...
/** @internal */
const initializeProviders = async (logger: Logger): Promise<CarbonCreditProviders> => {
  const networkId = import.meta.env.VITE_NETWORK_ID as NetworkId;
  const connectedAPI = await connectToWallet(logger, networkId);
  const zkConfigPath = window.location.origin; // '../../../contract/src/managed/carbon-credit';
  const keyMaterialProvider = new FetchZkConfigProvider<CarbonCreditCircuitKeys>(zkConfigPath, fetch.bind(window));
  const config = await connectedAPI.getConfiguration();
  const inMemoryCarbonCreditPrivateStateProvider = inMemoryPrivateStateProvider<string, CarbonCreditPrivateState>();
  const shieldedAddresses = await connectedAPI.getShieldedAddresses();
  return {
    privateStateProvider: inMemoryCarbonCreditPrivateStateProvider,
